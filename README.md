...
## Milestone 8 five-chapter qualification

Milestone 8 qualifies source chapters 2–6 only.
Their stable ordered scope is `chapter-0002`, `chapter-0003`, `chapter-0004`, `chapter-0005`, and `chapter-0006`.
The goal is one chaptered five-chapter M4B, not five separate chapter files and not a whole-book M4B.
...
Confirm that it names exactly five selected chapters and inspect every extraction warning, normalization warning, unresolved token, forced split, and chunk outlier.
...
`bilbo run` validates book-wide text artifacts, qualifies the five selected chapters, starts synthesis in the configured TTS Pixi environment, exits that process, runs ASR and bounded TTS retries in isolated processes, assembles one M4B, and publishes a deterministic build bundle.
...
Book-wide generation manifests may still identify missing audio outside chapters 2–6, which does not fail the selected C8 scope.

For `my-book`, the selected M4B path is:
...
The assembly report must show the exact five-chapter scope, five ordered chapter markers, no unaccepted override, and passing duration, metadata, loudness, true-peak, and FFprobe checks.
...
Rerun the exact five-chapter full command after recording the decision.
...
After a direct forced synthesis, rerun the full five-chapter command so verification, assembly, and bundle evidence are refreshed.

### Resume, rerun, and clean caches

After interruption or any recoverable failure, rerun the exact same five-chapter command.
