...
- Kokoro's demonstrated espeak-ng pronunciation defects are corrected in the shared [`config/lexicons/kokoro-it.yaml`](config/lexicons/kokoro-it.yaml) overlay; the reviewed `zero` exception uses a unique spoken marker that the pinned Kokoro adapter converts to the accepted `dzˈɛro` phoneme sequence after ordinary Italian G2P.
