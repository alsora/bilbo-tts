...
- Kokoro's demonstrated espeak-ng pronunciation defects are corrected in the shared [`config/lexicons/kokoro-it.yaml`](config/lexicons/kokoro-it.yaml) overlay; entries whose respelling cannot produce the reviewed pronunciation declare a `phoneme_override`, and the pinned Kokoro adapter replaces the marker's ordinary Italian G2P phonemes with that reviewed sequence.
- The adapter derives each marker's source phonemes by phonemizing the marker in isolation, which matches espeak-ng's clause-final rendering; where running context alters the marker's phonemes the ordinary rendering is kept as best-effort.
...
- The stable order is Unicode cleanup and dehyphenation; bounded equations; dates and structural references; ranges, percentages, currencies, ratios, and ordinals; abbreviations and symbols; decimals and generic integers; anni elision; reviewed lexicons; acronyms; canonical whitespace.
- Number expansion precedes lexicon application so reviewed pronunciation entries also correct spoken forms produced from digits, such as `1600` becoming `milleseicento`.
- Cardinals ending in venti, -anta, or cento elide before `anni` with a typographic apostrophe, so `30 anni` is spoken as `trent’anni` while non-eliding endings such as `venticinque anni` remain unchanged.
